...
const NoteOptionsActionSheet = ({noteId, formData}) => {
  ...
  const restoreNote = async () => {
    try {
      ...
      const newTrashNotesArr = trashNotes.filter(
        trashNote => trashNote.id !== note.id,
      );
  ...
  return (
    <>
      <ActionSheet
        ...
        id="noteOptionsActionSheet"
        useBottomSafeAreaPadding={false}
        defaultOverlayOpacity={0.1}
        openAnimationConfig={{friction: 20, tension: 250}}
        gestureEnabled
        ...
        <View>
          ...
          <ScrollView
            ...
            <Pressable
              ...
              {!note.color && <Icon name="checkmark" size={25} color="black" />}
            </Pressable>
          ...
          <ScrollView
            ...
            <Pressable
              onPress={() => {
                SheetManager.hide('noteOptionsActionSheet');
                navigation.navigate('NoteLabels', {noteId});
              }}
              style={{
                marginLeft: 14,
                backgroundColor: '#eee',
              ...
              <Text style={{fontWeight: '500', color: '#666', fontSize: 15}}>
                + Manage labels
              </Text>
            </Pressable>
          </ScrollView>
          ...
          {[
            ...
            {
              onPress: togglePin,
              iconName: note.isPinned ? 'pin' : 'pin-outline',
              title: note.isPinned ? 'Unpin' : 'Pin',
            },
            {
              onPress: showAddReminderModal,
              iconName: 'alarm-outline',
              title:
                newOrEditMode === 'edit' ? 'Edit reminder' : 'Add a reminder',
            },
            ...
            <Pressable
              ...
              android_ripple={{color: '#999', radius: 200}}>
              <Icon
                name={iconName}
                size={25}
                color="gray"
